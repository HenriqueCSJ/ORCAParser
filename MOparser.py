# ================================================================
# ORCA Frontier Orbital Analyzer (v5.1)
# 2025-05-25
# ================================================================
import math
import re
...
class AnalyzerConfig:
    ...
    occupancy_threshold: float = (
        0.1  # General threshold for considering an orbital "occupied"
    )
...
class Orbital:
    ...
    def energy_in_unit(
        self, unit: str = "eV", config: Optional[AnalyzerConfig] = None
    ) -> float:
    ...
    def is_occupied(
        ...
        bool
    ):  # Using a fixed threshold for now as AnalyzerConfig is not easily passed here
...
class OrbitalSet:
    ...
    def homo(self) -> Optional[Orbital]:
        ...
        occupied = [
            ...
            if orb.occupancy > self.config.occupancy_threshold
        ]
    ...
    def get_frontier_orbitals(self, n: int = 5) -> List[Orbital]:
        ...
        end_idx = min(
            len(self.orbitals), lumo_idx + n + 1
        )  # n above LUMO, +1 for inclusive slice end
...
class ElectronicDescriptors:
    ...
    @classmethod
    def from_frontier_orbitals(
        cls, homo: Optional[Orbital], lumo: Optional[Orbital], config: AnalyzerConfig
        ...
        softness = (
            1.0 / gap if abs(gap) > config.convergence_threshold else float("inf")
        )
    ...
    def to_dataframe(
        self, target_unit: str = "eV", config: Optional[AnalyzerConfig] = None
    ) -> pd.DataFrame:
        """Convert to formatted DataFrame, energies in target_unit."""
        conv_factor = ENERGY_CONVERSIONS.get(target_unit.lower(), 1.0)

        data = {
            f"ε HOMO ({target_unit})": (
                self.homo_energy_ev * conv_factor
                if self.homo_energy_ev is not None
                else None
            ),
            f"ε LUMO ({target_unit})": (
                self.lumo_energy_ev * conv_factor
                if self.lumo_energy_ev is not None
                else None
            ),
            f"Gap ({target_unit})": (
                self.gap_ev * conv_factor if self.gap_ev is not None else None
            ),
            f"IP ≈ −εH ({target_unit})": (
                self.ionization_potential_ev * conv_factor
                if self.ionization_potential_ev is not None
                else None
            ),
            f"EA ≈ −εL ({target_unit})": (
                self.electron_affinity_ev * conv_factor
                if self.electron_affinity_ev is not None
                else None
            ),
            f"μ ({target_unit})": (
                self.chemical_potential_ev * conv_factor
                if self.chemical_potential_ev is not None
                else None
            ),
            f"χ = −μ ({target_unit})": (
                self.electronegativity_ev * conv_factor
                if self.electronegativity_ev is not None
                else None
            ),
            f"η ({target_unit})": (
                self.hardness_ev * conv_factor if self.hardness_ev is not None else None
            ),
            f"S (1/{target_unit})": (
                self.softness_ev_inv / conv_factor
                if self.softness_ev_inv is not None and conv_factor != 0
                else None
            ),  # Softness unit is inverse energy
        }
...
class OrcaParser:
    ...
    _PATTERNS = {
        ...
        "orbital_banner": re.compile(
            r"^-+\s*ORBITAL\s+ENERGIES\s*-+", re.MULTILINE | re.IGNORECASE
        ),
        "spin_up": re.compile(
            r"^\s*SPIN\s+UP\s+ORBITALS\s*$", re.MULTILINE | re.IGNORECASE
        ),
        "spin_down": re.compile(
            r"^\s*SPIN\s+DOWN\s+ORBITALS\s*$", re.MULTILINE | re.IGNORECASE
        ),
        "column_header": re.compile(
            r"^\s*NO\s+OCC\s+E\(Eh\)\s+E\(eV\)(?:\s+Irrep)?\s*$", re.MULTILINE
        ),
    ...
    def parse(
        self, source: Union[str, Path], force_reparse: bool = False
    ) -> Dict[str, Optional[OrbitalSet]]:
        ...
        if isinstance(source, Path) or (
            ...
            if self.config.enable_caching and not force_reparse:
                cache_file = (
                    ...
                    / f"{file_path_obj.stem}_{self._get_file_hash(file_path_obj)}.pkl"
                )
                if cache_file.exists():
                    try:
                        ...
                        if self.config.verbose:
                            print(
                                f"📂 Loading '{source_name}' from cache: {cache_file}"
                            )
                        return cached_data
                    except Exception as e:
                        warnings.warn(
                            f"Cache read failed for {cache_file}: {e}. Reparsing."
                        )
            ...
            raise TypeError(
                "Source must be a file path (str or Path) or a raw text string."
            )
        ...
        try:
            ...
            if file_path_obj and self.config.enable_caching:
                ...
                try:
                    ...
                    if self.config.verbose:
                        print(f"💾 Cached '{source_name}' results to: {cache_file}")
                except Exception as e:
                    warnings.warn(f"Cache write failed for {cache_file}: {e}")
            return result
        except Exception as e:
            raise RuntimeError(
                f"Failed to parse ORCA output from '{source_name}': {str(e)}"
            ) from e
    ...
    def _parse_content(self, content: str) -> Dict[str, Optional[OrbitalSet]]:
        ...
        if not banner_matches:
            raise ValueError("No 'ORBITAL ENERGIES' section found.")
        ...
        if spin_up_match and spin_down_match:
            ...
            if self.config.verbose:
                warnings.warn(
                    "Found only SPIN UP orbitals. Treating as ROHF-like alpha channel."
                )
        else:
            results["restricted"] = self._parse_orbital_block(
                section_text, "restricted"
            )

        if not any(v for v in results.values() if v and v.orbitals):
            raise ValueError(
                "No orbital data lines could be parsed from any identified section."
            )
        return results
    ...
    def _parse_orbital_block(self, text: str, spin_type: str) -> Optional[OrbitalSet]:
        ...
        if not header_match:
            warnings.warn(
                f"No column header found in {spin_type} orbital block. Skipping."
            )
            return None
...
class OrbitalPlotter:
    ...
    def _setup_plotting(self):
        try:
            ...
            warnings.warn(
                f"Matplotlib style '{self.style.style_name}' not found. Using default."
            )
    ...
    def plot_single_spin(
        ...
        if not frontier_orbs:
            ...
            ax.text(0.5, 0.5, "No orbitals to display.", ha="center", va="center")
            ax.axis("off")
            return fig
        ...
        energies_for_plot = [
            orb.energy_in_unit(self.config.energy_units) for orb in frontier_orbs
        ]
        y_min, y_max = min(energies_for_plot) - 1, max(energies_for_plot) + 1

        for group in groups:
            self._plot_orbital_group(
                ax, group, x_center, level_width, text_offset, label_side="both"
            )  # 'both' for single plots

        descriptors = ElectronicDescriptors.from_frontier_orbitals(
            orbital_set.homo, orbital_set.lumo, self.config
        )
        if orbital_set.homo and orbital_set.lumo and descriptors.gap_ev is not None:
            self._add_gap_annotation(
                ax,
                orbital_set.homo.energy_in_unit(self.config.energy_units),
                orbital_set.lumo.energy_in_unit(self.config.energy_units),
                descriptors.gap_ev
                * ENERGY_CONVERSIONS.get(self.config.energy_units.lower(), 1.0),
                x_center,
            )

        if descriptors.chemical_potential_ev is not None:
            mu_plot_unit = descriptors.chemical_potential_ev * ENERGY_CONVERSIONS.get(
                self.config.energy_units.lower(), 1.0
            )
            self._add_chemical_potential_line(
                ax, mu_plot_unit, x_center, level_width + 0.1
            )  # slightly wider line

        ax.set_xlim(0, 1)  # Simpler xlim for single plot centered at 0.5
        ax.set_ylim(y_min, y_max)
        ax.set_ylabel(
            f"Energy ({self.config.energy_units})",
            fontsize=self.style.axis_label_fontsize,
            weight="bold",
        )
        ax.set_title(
        ...
        plt.tight_layout()
        if save_path or (
            self.config.auto_save_plots and not interactive
        ):  # Avoid saving if just for interactive display
            self._save_figure(
                fig,
                save_path
                or self._generate_filename(f"{orbital_set.spin_type}_orbitals"),
            )
        return fig
    ...
    def plot_combined_spins(
        ...
        fig, ax = plt.subplots(
            figsize=(10, 12)
        )  # Adjusted from 12,14 to 10,12 like single plots
        ...
        if alpha_set and alpha_set.orbitals:
            ...
            alpha_groups = self._group_degenerate_orbitals(
                alpha_frontier, degeneracy_threshold_ev
            )
            all_plot_energies.extend(
                [orb.energy_in_unit(self.config.energy_units) for orb in alpha_frontier]
            )
        if beta_set and beta_set.orbitals:
            beta_frontier = beta_set.get_frontier_orbitals(n_around)
            beta_groups = self._group_degenerate_orbitals(
                beta_frontier, degeneracy_threshold_ev
            )
            all_plot_energies.extend(
                [orb.energy_in_unit(self.config.energy_units) for orb in beta_frontier]
            )
        ...
        if alpha_groups:
            ...
            if (
                ...
                and alpha_set.homo
                and alpha_set.lumo
                and alpha_set.gap is not None
            ):
                gap_alpha_plot_unit = alpha_set.gap * ENERGY_CONVERSIONS.get(
                    self.config.energy_units.lower(), 1.0
                )
                self._add_gap_annotation(
                    ax,
                    alpha_set.homo.energy_in_unit(self.config.energy_units),
        ...
        if beta_groups:
            ...
            if (
                ...
                and beta_set.homo
                and beta_set.lumo
                and beta_set.gap is not None
            ):
                gap_beta_plot_unit = beta_set.gap * ENERGY_CONVERSIONS.get(
                    self.config.energy_units.lower(), 1.0
                )
                self._add_gap_annotation(
                    ax,
                    beta_set.homo.energy_in_unit(self.config.energy_units),
        ...
        if all_plot_energies:
            ...
            ax.text(
                0.5,
                0.5,
                "No alpha/beta orbitals found to plot.",
                ha="center",
                va="center",
        ...
        ax.text(
            ...
            ha="center",
            va="bottom",
            fontsize=self.style.axis_label_fontsize - 1,
            weight="bold",
            transform=ax.transAxes,
        )
        ax.text(
        ...
        ax.set_title(
            ...
            fontsize=self.style.title_fontsize,
            weight="bold",
            pad=30,  # Increased pad for spin labels
        )
        for spine in ["top", "right", "bottom"]:
            ax.spines[spine].set_visible(False)
        ax.spines["left"].set_linewidth(1.3)
        ...
        if save_path or self.config.auto_save_plots:
            self._save_figure(
                fig, save_path or self._generate_filename("combined_orbitals")
            )
        return fig

    def _group_degenerate_orbitals(
        self, orbitals: List[Orbital], threshold_ev: float
    ) -> List[Dict[str, Any]]:
        if not orbitals:
            return []
            ...
            return [
                {
                    ...
                    "is_occupied": orb.occupancy
                    > self.config.occupancy_threshold,  # Use config threshold
        ...
        for orb in sorted_orbitals:
            ...
            if not current_group_orbs or (
                abs(orb.energy_ev - np.mean([o.energy_ev for o in current_group_orbs]))
                <= threshold_ev
                and is_orb_occupied
                == (current_group_orbs[0].occupancy > self.config.occupancy_threshold)
            ):
                ...
                groups.append(
                    {
                        ...
                        "mean_energy_ev": np.mean(
                            [o.energy_ev for o in current_group_orbs]
                        ),
                        "is_occupied": current_group_orbs[0].occupancy
                        > self.config.occupancy_threshold,
                        "irreps": sorted(
                            list(
                                set(
                                    o.irrep
                                    for o in current_group_orbs
                                    if o.irrep != "N/A"
                                )
                            )
                        ),
            ...
            groups.append(
                {
                    ...
                    "irreps": sorted(
                        list(
                            set(o.irrep for o in current_group_orbs if o.irrep != "N/A")
                        )
                    ),
                }
            )
    ...
    def _plot_orbital_group(
        ...
        if not group.get("orbitals"):
            if self.config.verbose:
                print("⚠️ Warning: Attempting to plot empty orbital group.")
            return

        orbitals_in_group = group["orbitals"]
        # Energy for plotting should be in target units
        y_pos_plot_unit = group["mean_energy_ev"] * ENERGY_CONVERSIONS.get(
            self.config.energy_units.lower(), 1.0
        )
        is_group_occupied = group["is_occupied"]

        color = (
            (color_occupied or self.style.occupied_color)
            if is_group_occupied
            else (color_virtual or self.style.virtual_color)
        )
        ...
        if n_lines == 1:
            ax.hlines(
                ...
                line_area_end_x,
                color=color,
                lw=self.style.line_linewidth,
            )
        else:
            # Draw multiple short segments within the allocated width
            segment_total_width = (
                width * 0.8
            )  # Use 80% of width for segments to leave small gaps
            gap_between_segments = (
                segment_total_width * 0.1 / (n_lines - 1) if n_lines > 1 else 0
            )
            single_segment_width = (
                segment_total_width - gap_between_segments * (n_lines - 1)
            ) / n_lines
            ...
            for i in range(n_lines):
                ax.hlines(
                    y_pos_plot_unit,
                    current_x,
                    current_x + single_segment_width,
                    color=color,
                    lw=self.style.line_linewidth,
                )
        ...
        label_mo_info = (
            f"MO {numbers[0]}"
            if len(numbers) == 1
            else f"MO {numbers[0]}-{numbers[-1]}"
        )
        if group.get("irreps"):
            label_mo_info += f" ({'/'.join(group['irreps'])})"
        if len(orbitals_in_group) > 1:
            label_mo_info += f" (×{len(orbitals_in_group)})"

        # Energy value string (always prepared, used by 'both')
        energy_val_str = (
            f"{group['mean_energy_ev']:.2f}"  # Use 2 decimal places for eV in plot
        )
        ...
        current_fontsize = (
            self.style.mo_label_fontsize_single
            if label_side == "both"
    ...
    def _add_gap_annotation(
        self,
        ax: plt.Axes,
        homo_energy_plot_unit: float,
        lumo_energy_plot_unit: float,
        gap_plot_unit: float,
        x_center: float,
        label_prefix: str = "",
    ):
        ax.annotate(
            "",
            xy=(x_center, homo_energy_plot_unit),
            xycoords="data",
            xytext=(x_center, lumo_energy_plot_unit),
            textcoords="data",
            arrowprops=dict(
                arrowstyle="<->",
                lw=self.style.arrow_linewidth_main,
                color=self.style.gap_arrow_color,
                shrinkA=0,
                shrinkB=0,
            ),
        )
        ax.text(
            x_center,
            (homo_energy_plot_unit + lumo_energy_plot_unit) / 2,
            f" {label_prefix}Gap = {gap_plot_unit:.2f} {self.config.energy_units} ",  # Use 2 decimal places
            ha="center",
            va="center",
            weight="bold",
            fontsize=self.style.annotation_fontsize_main,
            bbox=dict(
                boxstyle="round,pad=0.25", fc=self.style.gap_text_bgcolor, alpha=0.9
            ),
        )

    def _add_chemical_potential_line(
        self,
        ax: plt.Axes,
        mu_plot_unit: float,
        x_center: float,
        line_total_width: float,
    ):
        # line_total_width is the span the dashed line should cover
        xmin_frac = (x_center - line_total_width / 2) / (
            ax.get_xlim()[1] - ax.get_xlim()[0]
        )  # Normalize based on current xlim
        xmax_frac = (x_center + line_total_width / 2) / (
            ax.get_xlim()[1] - ax.get_xlim()[0]
        )
        ...
        ax.axhline(
            mu_plot_unit,
            color=self.style.mu_line_color,
            ls=":",
            lw=self.style.arrow_linewidth_main - 0.3,
            xmin=(
                (line_start_abs - plot_xmin) / (plot_xmax - plot_xmin)
                if (plot_xmax - plot_xmin) != 0
                else 0.05
            ),
            xmax=(
                (line_end_abs - plot_xmin) / (plot_xmax - plot_xmin)
                if (plot_xmax - plot_xmin) != 0
                else 0.95
            ),
        )
        ...
        ax.text(
            x_center + line_total_width / 2 + 0.02,
            mu_plot_unit,  # Small offset for text from line end
            f"μ = {mu_plot_unit:.2f} {self.config.energy_units}",  # Use 2 decimal places
            ha="left",
            va="center",
            fontsize=self.style.annotation_fontsize_small,
            bbox=dict(
                boxstyle="round,pad=0.15", fc=self.style.mu_text_bgcolor, alpha=0.8
            ),
        )
    ...
    def _save_figure(self, fig: plt.Figure, path: Union[str, Path]):
        ...
        if not path_obj.suffix or path_obj.suffix.lower() not in [
            ".png",
            ".pdf",
            ".svg",
            ".jpg",
            ...
            final_path_str = str(
                path_obj.with_suffix(f".{self.config.plot_format.lower()}")
            )

        fig.savefig(final_path_str, dpi=self.style.figure_dpi, bbox_inches="tight")
        if self.config.verbose:
            print(f"💾 Plot saved to: {final_path_str}")

    def _generate_filename(self, plot_type: str) -> Path:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        return (
            self.config.export_dir / f"orbitals_{plot_type}_{timestamp}"
        )  # Suffix added by _save_figure

    def _add_interactivity(
        self, fig: plt.Figure, ax: plt.Axes, orbitals: List[Orbital]
    ):
        # Placeholder for future matplotlib interactive features
        if self.config.verbose:
            print("ℹ️ Interactive mode placeholder.")
        pass
...
class OrcaAnalyzer:
    def __init__(
        self, config: Optional[AnalyzerConfig] = None, style: Optional[PlotStyle] = None
    ):
    ...
    def analyze(
        ...
        try:
            ...
            results: Dict[str, Any] = {
                ...
                "orbital_data_raw": {
                    spin: (oset.orbitals if oset else [])
            ...
            for spin_type in ["restricted", "alpha", "beta"]:
                ...
                if orbital_set and orbital_set.orbitals:  # Ensure there are orbitals
                    self._analyze_spin_channel(
                        ...
                        orbital_set,
                        orbitals_around,
                        degeneracy_threshold_ev,
                        _export_plots,
                        interactive,  # Pass effective export plot flag
                    )
            ...
            if alpha_set and alpha_set.orbitals and beta_set and beta_set.orbitals:
                self._analyze_unrestricted(
                    ...
                    orbitals_around,
                    degeneracy_threshold_ev,
                    _export_plots,  # Pass effective export plot flag
                )
            ...
            error_msg = (
                f"Analysis failed for '{str(source)}': {type(e).__name__} - {str(e)}"
            )
            if self.config.verbose:
                import traceback
    ...
    def _analyze_spin_channel(
        ...
        degen_thresh: float,
        export_plt: bool,
        interactive_plt: bool,
    ):
        if self.config.verbose:
            print(f"\n📊 Analyzing {spin_type} orbitals...")

        desc = ElectronicDescriptors.from_frontier_orbitals(
            orbital_set.homo, orbital_set.lumo, self.config
        )
        results_dict["descriptors"][spin_type] = desc

        if self.config.verbose:
            self._display_spin_results(spin_type, orbital_set, desc)

        save_path_obj = None
        if export_plt:
            save_path_obj = self.plotter._generate_filename(
                f"{results_dict['source_name']}_{spin_type}"
            )
        ...
        fig = self.plotter.plot_single_spin(
            ...
            interactive=interactive_plt,
        )
        results_dict["plots"][spin_type] = (
            fig if not interactive_plt else "Interactive plot shown"
        )
        if not interactive_plt:
            plt.show()  # Show non-interactive plots
    ...
    def _analyze_unrestricted(
        ...
        possible_homos = [
            orb
            for orb_set in (alpha_s, beta_s)
            if orb_set.homo
            for orb in [orb_set.homo]
        ]
        possible_lumos = [
            orb
            for orb_set in (alpha_s, beta_s)
            if orb_set.lumo
            for orb in [orb_set.lumo]
        ]

        overall_homo = (
            max(possible_homos, key=lambda o: o.energy_ev) if possible_homos else None
        )
        overall_lumo = (
            min(possible_lumos, key=lambda o: o.energy_ev) if possible_lumos else None
        )

        overall_desc = ElectronicDescriptors.from_frontier_orbitals(
            overall_homo, overall_lumo, self.config
        )
        ...
        if (
            ...
            and overall_homo
            and overall_lumo
            and overall_desc.gap_ev is not None
        ):
            print(
                f"  Overall HOMO ({overall_homo.spin if overall_homo else 'N/A'}): {overall_homo.energy_ev:.3f} eV"
            )
            print(
                f"  Overall LUMO ({overall_lumo.spin if overall_lumo else 'N/A'}): {overall_lumo.energy_ev:.3f} eV"
            )
        ...
        if export_plt:
            save_path_obj = self.plotter._generate_filename(
                f"{results_dict['source_name']}_combined"
            )
    ...
    def _display_spin_results(
        self, spin_t: str, orb_set: OrbitalSet, desc: ElectronicDescriptors
    ):
        print(
        ...
        if orb_set.homo:
            h = orb_set.homo
            print(
                f"HOMO: MO {h.number} ({h.irrep}) {h.energy_in_unit(unit):.3f} {unit} (Occ: {h.occupancy:.3f})"
            )
        else:
            print("HOMO: Not found")
        if orb_set.lumo:
            l = orb_set.lumo
            print(
                f"LUMO: MO {l.number} ({l.irrep}) {l.energy_in_unit(unit):.3f} {unit} (Occ: {l.occupancy:.3f})"
            )
        else:
            print("LUMO: Not found")

        if desc.homo_energy_ev is not None:
            display(
                HTML(desc.to_dataframe(target_unit=unit)._repr_html_())
            )  # Use HTML for better display

    def _generate_summary(
        self, results_dict: Dict, orbital_sets: Dict[str, Optional[OrbitalSet]]
    ):
        summary = {
            "calculation_type": (
                "restricted"
                if orbital_sets.get("restricted")
                and orbital_sets["restricted"].orbitals
                else "unrestricted"
            ),
            "spin_channels_found": [
                k for k, v in orbital_sets.items() if v and v.orbitals
            ],
            "gaps_ev": {},
        }
        for spin, desc in results_dict["descriptors"].items():
            if hasattr(desc, "gap_ev") and desc.gap_ev is not None:
                summary["gaps_ev"][spin] = desc.gap_ev

        if summary["gaps_ev"]:
            min_gap_spin = min(summary["gaps_ev"], key=summary["gaps_ev"].get)
            summary["smallest_gap_ev"] = {
                "value": summary["gaps_ev"][min_gap_spin],
                "spin": min_gap_spin,
            }
    ...
    def _export_results(
        self,
        results: Dict,
        orbital_sets: Dict[str, Optional[OrbitalSet]],
        formats: List[str],
    ):
        base_name = self.plotter._generate_filename(
            results["source_name"]
        ).stem  # Get timestamped base
        ...
        if "json" in formats:
            ...
            serializable_results = {
                ...
                "orbital_dataframes": {
                    k: (
                        oset.to_dataframe().to_dict(orient="records")
        ...
        if "excel" in formats:
            ...
            with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
                ...
                for spin, oset in orbital_sets.items():
                    if oset and oset.orbitals:
                        oset.to_dataframe().to_excel(
                            writer,
                            sheet_name=f"{spin.capitalize()} Orbitals",
                            index=False,
                        )
                ...
                if desc_list:
                    pd.concat(desc_list).set_index(
                        ["Spin_Channel", "Descriptor"]
                    ).to_excel(writer, sheet_name="Descriptors")
            if self.config.verbose:
                print(f"💾 Excel data exported to: {excel_path}")

    def compare_files(
        self, file_paths: List[Union[str, Path]], metric: str = "gap_ev"
    ) -> pd.DataFrame:
        # Simplified comparison focusing on a few key metrics
        comp_data = []
        prog_bar = tqdm(
            file_paths, desc="Comparing files", disable=not self.config.show_progress
        )
        for f_path in prog_bar:
            prog_bar.set_postfix_str(Path(f_path).name)
            res = self.analyze(
                f_path, export_data_formats=None, export_plot_formats=None
            )  # No exports during compare
            if "error" in res:
                comp_data.append({"file": Path(f_path).name, "error": res["error"]})
                continue

            row = {
                "file": Path(f_path).name,
                "type": res["summary"]["calculation_type"],
            }
            for spin, desc in res["descriptors"].items():
                if hasattr(desc, metric):
                    row[f"{spin}_{metric}"] = getattr(desc, metric)
...
def test_plot_generation(analyzer: OrcaAnalyzer, spin_type: str):
    ...
    if spin_type == "combined":
        ...
        analyzer.plotter.plot_combined_spins(
            alpha_set, beta_set, n_around=2, degeneracy_threshold_ev=0.05
        )
    else:  # restricted or single alpha/beta
        orbs = [
            Orbital(
                n,
                2.0 if n < 3 and spin_type == "restricted" else (1.0 if n < 3 else 0.0),
                -0.5 + 0.05 * n,
                (-0.5 + 0.05 * n) * 27.2,
                "A1g",
                spin_type,
        ...
        analyzer.plotter.plot_single_spin(
            orb_set, n_around=2, degeneracy_threshold_ev=0.05
        )
...
if __name__ == "__main__":
    ...
    style_user = PlotStyle(
        figure_dpi=120
    )  # Slightly lower DPI for faster display if needed
    ...
    if orca_file_input or orca_text_input:
        ...
        analysis_results = analyzer.analyze(
            ...
            export_data_formats=[
                "json",
                "excel",
            ],  # Specify desired data formats for export
            export_plot_formats=[
                "png"
            ],  # Specify desired plot formats for export (if auto_save_plots is False, these still trigger named save)
        )
